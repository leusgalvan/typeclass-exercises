...
trait EqLaws[A] {
  ...
  // TODO #10: Definir la propiedad 'reflexivity' que chequea que un valor es igual a si mismo
  def reflexivity(a: A): Boolean = eq.eq(a, a)

  // TODO #11: Definir la propiedad 'symmetry' que chequea que la igualdad entre 'x' e 'y' es equivalente a la
  //           igualdad entre 'y' y 'x'.
  def symmetry(x: A, y: A): Boolean = eq.eq(x, y) == eq.eq(y, x)

  // TODO #12: Definir la propiedad 'transitivity' que chequea que si 'x' es igual a 'y', e 'y' es igual a 'z',
